...
The 'City Official Map' is the city official requested map, which provides an overview of all the stations for the current and previous month.
...
The 'Context Map - Above the 90th Percentile' page of the story confirms the pattern described above. It demonstrates that stations with counts above the 90th percentile (approximately 5000 trips) are mostly in Manhattan.
...
The first phenomena explores the relationship between this fact as well as the trip validity (short, valid, long) and membership type.

__Observation 3__

There were trips that started and ended outside the New York City limits. The second phenomena explores this, identifying the trends in the system boundary to determine whether more trips flow in or out of the New York City Limits

__Observation 4__

One station can have multiple latitude and longitude combinations. The workflow simplified this by taking an average for all stations, so that there is one set of coordinates (latitude, longitude) per station.
...
2. All Top 10 Stations (both Start and End) had usage between 8,000 and 15,000 trips per station. Assuming a 30-day month, for each of these stations, this would equate to approximately 250 to 500 trips per day. This is a key observation to make, to fully appreciate the breakdown of valid trips in the first phenomena.
...
The first table on the right, 'Trip Validity vs Membership' denotes the breakdown over the Sep-Oct period.
...
In terms of the map in this dashboard:
...
The bar charts on the bottom all demonstrate the flow of bikes in and out of the city. Overall, there has been a net flow of bikes out of the city. This is demonstrated by the much larger count for "End Stations" rather than "Start Stations".
